...
def modified_Resnet_se(n_filters, input_layer):
    init = RandomNormal(stddev=0.02)
...
class CNN(keras.Model):
    ...
    def _get_1dcnn(self):
        ...
        enc = modified_Resnet_eca(64, enc)
        enc = modified_Resnet_eca(64, enc)
        # # (None,16,32)
        # enc=gen_block_down(64,5,2,'same',enc)
        # (None, 128)
    ...
    def train(self, train_db, test_db, epochs):
        ...
        # np.savetxt ('./results/cnn_resnet_eca/res_eca_650.txt',af)
        # np.savetxt('./results/cnn_resnet_eca/res_eca_loss_650.txt',los)
        np.savetxt('./results/cnn_resnet_eca/pred.txt', pred)
        np.savetxt('./results/cnn_resnet_eca/label.txt', label)
        print(label)
        print(pred)
...
cnn = CNN(input_shape)
cnn.train(train_db, test_db, epochs=56)
