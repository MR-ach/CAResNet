...
# labels_=[]
for fault in faults:
    with open(path + fault + '_test.pkl', 'rb') as f:
        data,_=pickle.load(f)
        dataset.append(data)
datasets=np.array(dataset).reshape(data.shape[0]*len(faults),data.shape[1])
# print(datasets)

label1=np.array([1]*300).reshape(300,1)
label2=np.array([2]*300).reshape(300,1)
label3=np.array([3]*300).reshape(300,1)
...
# tensroflow图
a=1 #lamda 设置为： 1 0.1 0.01 0.001
# 设置占位符
...
train_log_dir = './model/model_class_'+str(a)+'/model.ckpt' #训练保存路径
