...
for a in lamda:
    ...
    labels = np.repeat([np.array([1, 0, 0])], 300, axis=0)
    print(labels)
    ...
    # 控制输出
    y_slect = labels[0:2]
    num_examples_to_eval = 300
    name_generated = 'generated_C' + faults + '_test_seed_' + a

    with tf.Session() as sess:
        # 加载保存的模型
        new_saver = tf.train.import_meta_graph(
            './model/model_class_' + a + '/model.ckpt-19000.meta')
        new_saver.restore(sess, tf.train.latest_checkpoint(
            './model/model_class_' + str(a)))
