    ...
    def train(self, train_db, test_db, epochs):
        for epoch in range(epochs):
            ...
            for x_T, y_T in test_db:
                ...
                pred_y_T1 = tf.argmax(pred_y_T, axis=-1)
                y_T = tf.cast(y_T, tf.int64)
                correct += float(tf.reduce_sum(tf.cast(tf.equal(pred_y_T1, y_T), tf.float32)))
            ...
            pred.append(pred_y_T)

            print('epoch:', epoch, 'loss:', loss.numpy(), 'test acc:', correct / total)
        # tf.reduce_mean(af)
        # np.savetxt ('./results/cnn_se/se_30.txt',af)
        # np.savetxt('./results/cnn_se/se_loss_30.txt', los)
        # np.savetxt('./results/cnn_se/pred.txt', pred)
